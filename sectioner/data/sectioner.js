...
(function (window) {
    ...
    var URL_ROOT = (function getRoot() {
        var withPageRe = new RegExp('^(.*\/).+\.html');
        var res = withPageRe.exec(document.location.pathname);
        if (res) {
            return res[1];
        }
    ...
    SImage.prototype.load = function () {
        ...
        node.setAttribute('src', absoluteURL(source));
    };

    SImage.prototype.setBookmark = function () {
        this.bookmark = this.offset;
    };

    SImage.prototype.resetBookmark = function () {
    ...
    Slider.prototype.at = function (index) {
        ...
        var gap = this.gap;
        this.images.forEach(function (im, i, images) {
            var baseOffset = images.slice(0, i).reduce(function (acc, img) {
                return img.width + acc;
            }, i * gap);
            console.log(`${baseOffset} ${offset} ${baseOffset - offset}`)
            im.setOffset(baseOffset - offset, {
                duration: SPEED.MEDIUM
            });
        });
    ...
    Slider.prototype.forEachImage = function (fn) {
        this.images.forEach(fn);
    };
    ...
    Pager.prototype.start = function (pos) {
        ...
        this.each(function (page) {
            page.setBookmark();
            if (page.slider) {
                page.slider.forEachImage(function (image) {
                    image.setBookmark();
                });
            }
        });
        ...
        if (this.isStarted) {
            ...
            if (tsDiff < 200) {
                ...
                if (DIRECTION.HORIZONTAL === dir) {
                    ...
                    if (slider) {
                        var cIdx = slider.index;
                        var resetAll = function () {
                            slider.forEachImage(function (image) {
                                image.resetBookmark();
                            });
                        };
                        var next = (cIdx === slider.images.length - 1)
                            ? resetAll
                            : slider.next;
                        var prev = (cIdx === 0)
                            ? resetAll
                            : slider.previous;
                        var hasMoved = this.checkOffset(offset, dir, next, prev, slider);
                    ...
                    var cIdx = this.index;
                    var resetAll = _.bind(function () {
                        this.each(function (page) {
                            page.resetBookmark();
                        });
                    }, this);
                    var next = (cIdx === this.pages.length - 1)
                        ? resetAll
                        : this.pageNext;
                    var prev = (cIdx === 0)
                        ? resetAll
                        : this.pagePrevious;
                    var hasMoved = this.checkOffset(
                        offset, dir, next, prev);
    ...
    Pager.prototype.touchmove = function (event) {
        ...
        if (touch) {
            ...
            if (DIRECTION.VERTICAL === dir) {
                ...
                var slider = this.getCurrentPage().slider;
                slider.forEachImage(function (image) {
                    image.translate(delta[DIRECTION.HORIZONTAL]);
                })
            }
    ...
    Router.prototype.push = function () {
        ...
        if (this.history) {
            var page = this.pager.getCurrentPage(),
                ...
                slug = pageData['slug'];
            this.history.pushState(page.index, title,
                absoluteURL(slug + '.html'));
        }
    };
